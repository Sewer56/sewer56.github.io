...
File: 6.78bpb, 9429814 LZ, 14061599/13420865/20015319 (70.25%/67.05%/100.00%) (est/zstd/orig)
...
This statistic is provided so you can tune your size predictor, in case you are writing transforms,
when you need to [***quickly*** estimate the file size][quickly-estimate-file-size].

!!! tip "For best results of course, do a two pass with the compression algorithm you'll be using."

    The methods described in the previous blog post are a fast estimate, there are occasional rare
    cases where the result defies expectations.

    !!! note "ZStd level 7 for example works great as a predictor for even level >= 16"

        So if real-time recompression is not a requirement, then you can spend a little longer.
